...
#define MAX_HANDOVER_WAIT_TIME_MS 30000
                          ...
                          GrdRemoteClient                 *remote_client)
{
  ...
  grd_session_rdp_send_server_redirection (GRD_SESSION_RDP (remote_client->session),
                                           routing_token,
                                           user_name, password);
                         ...
                         const char      *session_id)
{
  ...
  remote_client->dbus_object_path = g_strdup_printf (
                                      "%s/session%s",
                                      REMOTE_DESKTOP_HANDOVERS_OBJECT_PATH,
                                      session_id);

  remote_client->handover_skeleton = g_dbus_object_skeleton_new (
...
on_system_grd_name_lost (GDBusConnection *connection,
                         ...
                         gpointer         user_data)
{
  g_debug ("[DaemonSystem] Lost owning %s name", name);
}

static void
on_remote_display_factory_name_owner_changed (
  GrdDBusGdmRemoteDisplayFactory *remote_display_factory_proxy,
  GParamSpec                     *pspec,
  GrdDaemonSystem                *daemon_system)
{
...
grd_daemon_system_shutdown (GApplication *app)
{
  ...
  g_hash_table_remove_all (daemon_system->remote_clients);
  g_clear_pointer (&daemon_system->remote_clients,
                   g_hash_table_unref);
